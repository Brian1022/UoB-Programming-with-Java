...
public final class GameClient {
    ...
    private static void handleNextCommand(String username) throws IOException {
        System.out.print(new StringBuilder(username).append(":> ").toString());
        ...
        var socketWriter = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream()))) {
            socketWriter.write(new StringBuilder(username).append(": ").append(command).append("\n").toString());
            socketWriter.flush();
            String incomingMessage = socketReader.readLine();
            if (incomingMessage == null) {
                throw new IOException("Server disconnected (end-of-stream)");
            }
            while (incomingMessage != null && !incomingMessage.contains(new StringBuilder("").append(END_OF_TRANSMISSION).append(""))) {
